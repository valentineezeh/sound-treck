import React, { useEffect, useState, useRef, useCallback } from "react";
import { useDispatch, connect } from "react-redux";
import PropTypes from "prop-types";
import Loader from "react-loader-spinner";
import { PageLayout } from "layouts/page";
import SliderItem from "components/sliderItem";
import { getMusicians } from "store/actions";
import { _isObjectEmpty } from "utils";
import { ReactComponent as ArtistIcon } from "assets/img/svg/artists.svg";

const Artist = props => {
  const [pageNumber, setPageNumber] = useState(1);
  const [isLoading, setIsLoading] = useState(false);
  const [hasMore, setHasMore] = useState(false);
  const [mainArtists, setMainArtists] = useState([]);

  const observer = useRef();
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getMusicians(pageNumber));
  }, [dispatch, pageNumber]);

  useEffect(() => {
    if (!_isObjectEmpty(props.artists)) {
      setHasMore(props.artists.length > 0);
      setMainArtists(previousArtists => {
        return [...new Set([...previousArtists, ...props.artists])];
      });
    }
  }, [props.artists]);

  const lastAlbumElementRef = useCallback(
    node => {
      if (isLoading) return;
      if (observer.current) observer.current.disconnect();
      observer.current = new IntersectionObserver(payload => {
        if (payload[0].isIntersecting && hasMore) {
          setPageNumber(prevPageNumber => prevPageNumber + 1);
        }
      });
      if (node) observer.current.observe(node);
    },
    [isLoading, hasMore]
  );

  return (
    <PageLayout hasSidebar pageClass="dashboard dashboard--guest">
      <section className="section">
        <div className="container">
          <div className="section__header">
            <h3 className="heading">Artists</h3>
          </div>
        </div>
      </section>

      <section className="section">
        <div className="container">
          <div className="row">
            {mainArtists.length === 0 && (
              <div className="col-12 justify-content-center">
                <div className="empty-list">
                  <ArtistIcon className="empty-list-icon" />
                  <p className="paragraph mt-3">No Artists available</p>
                </div>
              </div>
            )}
            {mainArtists.map((artist, index) => {
              if (mainArtists.length === index + 1) {
                return (
                  <div
                    className="col-6 col-md-4 col-lg-3 col-xl-2 mb-5"
                    key={index}
                    ref={lastAlbumElementRef}
                  >
                    <SliderItem
                      key={index}
                      item={artist}
                      noArtist
                      type="artist"
                    />
                  </div>
                );
              }
              return (
                <div
                  className="col-6 col-md-4 col-lg-3 col-xl-2 mb-5"
                  key={index}
                >
                  <SliderItem
                    key={index}
                    item={artist}
                    noArtist
                    type="artist"
                  />
                </div>
              );
            })}

            {props.loading ? (
              <div className="col-12 justify-content-center">
                <div className="text-center mr-5">
                  <Loader
                    type="Circles"
                    color="#d3ae3b"
                    height={100}
                    width={100}
                  />
                </div>
              </div>
            ) : null}
          </div>
        </div>
      </section>
    </PageLayout>
  );
};

const mapStateToProps = state => ({
  artists: state.musicians.data,
  loading: state.musicians.loading
});

Artist.propTypes = {
  artists: PropTypes.shape({}),
  loading: PropTypes.bool
};

export default connect(
  mapStateToProps,
  null
)(Artist);
